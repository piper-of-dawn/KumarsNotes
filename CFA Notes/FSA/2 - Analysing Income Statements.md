...
1. Revenue recognition is a five-step test for deciding how much sales revenue a company is allowed to record, and when it is allowed to record it.
   ...
   - Remember: **Contract, promises, price, split price, book revenue.** Like a restaurant bill: first confirm the order, then list the dishes, total the bill, split it if needed, and only call it earned when the food is served.
2. Before you even think about revenue, ask whether IFRS or U.S. GAAP treats the customer agreement as a real **contract**. A contract exists only if collecting the money is **probable**.
   ...
   - Remember: **IFRS is the lower collectability hurdle; U.S. GAAP is the higher hurdle.** Same customer, same deal, different answer.
...
4. Principal vs agent presentation is about whether the company is the real seller or just the middleman.
   ...
   - Amazon acting as the seller of its own inventory is closer to principal. If Amazon buys a speaker for USD 70 and sells it for USD 100, it reports USD 100 of revenue, USD 70 of cost of sales, and USD 30 of gross profit.
   - Amazon Marketplace acting as a platform for a third-party seller is closer to agent. If the customer pays USD 100 to the seller and Amazon earns a USD 30 commission, Amazon reports USD 30 of revenue, not USD 100.
...
5. Franchising and licensing split the business into different revenue buckets. Do not treat all store sales as the parent company’s revenue.
   - If McDonald's owns and operates a restaurant, McDonald's reports the full burger-and-fries sale as revenue.
   - If a franchisee owns the McDonald's restaurant, the franchisee records the customer sale. McDonald's records only its royalty or fee from the franchisee, not the full restaurant sales.
...
6. Software revenue depends on whether the customer gets a software license or an ongoing software service.
   ...
   - Microsoft selling a traditional Office license is closer to upfront revenue because the customer receives the software right away. Microsoft 365 is closer to over-time revenue because the customer keeps receiving cloud access, updates, and service.
   - Netflix does not sell you a movie file forever. It gives you streaming access month by month, so the revenue pattern is over the subscription period.
...
7. Long-term construction and production contracts recognize revenue over time when control passes to the customer as work is performed.
   ...
   - Boeing or Lockheed Martin may build a custom aircraft or defense system over several years. If the customer controls the work as it is built and must pay for work completed, revenue is recognized over time, not only when the final aircraft or system is delivered.
...
10. You look more profitable **as long as you’re piling up assets faster than you’re wearing them out**. Once growth slows or assets mature, the bill arrives via depreciation.

> [!info] CAPITALIZE VS EXPENSE 
...
11. Expense recognition is about matching cost to the period that consumed the benefit.
   - What is expense recognition: The accounting decision about when spending becomes an expense on the income statement.
   - Why expense recognition is used: It stops companies from showing profit in one period while hiding the related cost in another period.
   - Example: If Tata Motors sells a car today, the cost of that specific car should hit cost of goods sold today, even if the inventory was bought last quarter.

2. The matching model is used when the expense directly helped create specific revenue.
   - What is matching: Put the cost in the same period as the revenue it helped generate.
   - Inventory is the cleanest exam case. Inventory first sits as an asset. When the product is sold, the inventory cost becomes cost of goods sold.
   - Problem: A retailer buys 7,600 units for USD 321,600, sells 5,600 units for USD 50 each, and identifies that the remaining 2,000 units cost USD 89,800.
   - Solution:
...
3. Period costs are expensed as incurred because they do not connect neatly to one specific sale.
...
4. Capitalization first, then depreciation or amortization later, is used when spending creates a long-term asset.
...
5. Capitalize versus expense changes timing, not the total pre-tax cost over the asset life.
...
> [!tip] RATIO EFFECTS - CAPITALIZE VS EXPENSE
...
6. The capitalization benefit continues only while new capitalized spending exceeds depreciation or amortization.
   - Imagine Jio buys one USD 300 network device every year, and each device is depreciated over three years at USD 100 per year.
   - Year 1 depreciation is USD 100, so capitalization makes profit USD 200 higher than immediate expensing.
   - Year 2 depreciation is USD 200, so the benefit shrinks.
   - Year 3 depreciation is USD 300, exactly equal to the new USD 300 device bought that year. The profit boost disappears.
   - So do not memorize "capitalization always boosts profit." It boosts profit only while the company is adding assets faster than old assets are being expensed.

7. Depreciation method and useful-life estimates are not boring footnotes; they move profit.
   - Longer useful life means lower annual depreciation and higher current profit.
   - Higher salvage value means lower depreciable cost and higher current profit.
   - Accelerated depreciation means higher early depreciation, lower early net income, and, if tax reporting follows the same method, higher operating cash flow because taxes are paid later.
   - Impairment loss means the asset was written down because its carrying value was too high. It lowers current net income but does not reduce current operating cash flow because it is non-cash.
   - Exam point: An impairment often hints that past depreciation was too low and past profits were overstated.

8. Capitalized interest is debt cost hidden inside an asset.
   - What is capitalized interest: Interest cost added to the cost of a long-construction asset instead of being reported immediately as interest expense.
   - If Reliance builds a refinery for its own use, capitalized interest becomes part of property, plant, and equipment and later appears through depreciation.
...
=
\frac{\text{Earnings before interest and taxes} + \text{amortization or depreciation of previously capitalized interest}}
{\text{Interest expense} + \text{current-period capitalized interest}}
$$
...
9. Capitalized interest also distorts cash flow classification.
...
10. Software development costs are a common margin case because "technological feasibility" requires judgment.
...
> [!warning] SOFTWARE ADJUSTMENT
> To compare a capitalizing software company with one that expenses development:
> - Add current software development spending to expenses.
> - Remove amortization of old capitalized software from expenses.
> - Reduce assets and equity by the capitalized software balance.
> - Reduce operating cash flow by current software development spending, and reduce investing cash outflow by the same amount.
>
...
11. Expense estimates are where management can quietly shape earnings.
